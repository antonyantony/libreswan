		...
		if (st == NULL) {
			...
			if (this->host->config->xauth.server &&
				...
				dbg("received isakmp_xchg_type %s; this is a%s%s%s%s in state %s. Reply with UNSUPPORTED_EXCHANGE_TYPE",
				    enum_show(&ikev1_exchange_names, md->hdr.isa_xchg, &b),
				    st->st_connection ->spd.this.host->config->xauth.server ? " xauthserver" : "",
				    st->st_connection->spd.this.host->config->xauth.client ? " xauthclient" : "",
			...
			if (st->st_connection->spd.this.host->config->xauth.server &&
			    IS_V1_PHASE1(st->st_state->kind)) {
...
void complete_v1_state_transition(struct state *st, struct msg_digest *md, stf_status result)
{
	...
	case STF_OK:
	{
		 ...
		 */
		if (st->st_connection->spd.this.host->config->xauth.client &&
		...
		if (c->spd.this.host->config->xauth.client &&
		    st->hidden_variables.st_xauth_client_done &&
		...
		case EVENT_SA_REPLACE: /* SA replacement event */
		{
			...
			if (IS_V1_PHASE1(st->st_state->kind) ||
				 ...
				 */
				event_delay = c->sa_ike_max_seconds;
				 ...
				 */
				event_delay = c->sa_ipsec_max_seconds;
