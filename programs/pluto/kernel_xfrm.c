...
static void netlink_kernel_sa_expire(struct nlmsghdr *n, struct logger *logger)
{
	...
	// dbg("%s spi 0x%x src %s dst %s %s mode %u proto %d bytes %" PRIu64 "packets %" PRIu64,
	//  error: format ‘%lu’ expects argument of type ‘long unsigned int’, but argument 9 has type ‘__u64’ {aka ‘long long unsigned int’} [-Werror=format=]
	//  bytes and packets complain about %PRIu64 FIX llu to work on 32bit ???
...
static bool netlink_get(int fd, struct logger *logger)
{
	...
	dbg("kernel: netlink_get: %s message with length %zu",
	    sparse_val_show(xfrm_type_names, rsp.n.nlmsg_type),
	    (size_t) rsp.n.nlmsg_len);
