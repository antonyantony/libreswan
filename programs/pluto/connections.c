			 ...
			 const struct connection *c)
{
	...
	readable_humber(c->sa_max_bytes, bytesbuf, bytesbuf + sizeof(bytesbuf), "", "B");
	readable_humber(c->sa_max_packets, packetsbuf, packetsbuf + sizeof(packetsbuf), "", "B");
	show_comment(s, PRI_CONNECTION":   ike_life: %jds; ipsec_life: %jds; replay_window: %u; rekey_margin: %jds; rekey_fuzz: %lu%%; keyingtries: %lu; ipsec_max_bytes: %s; ipsec_max_packets: %s;",
		     c->name, instance,
		     deltasecs(c->sa_ike_life_seconds),
		     deltasecs(c->sa_ipsec_life_seconds),
		     c->sa_replay_window,
		     deltasecs(c->sa_rekey_margin),
		     c->sa_rekey_fuzz,
		     c->sa_keying_tries, bytesbuf, packetsbuf);
