...
#define CONNECTION_POLICIES (POLICY_NEGO_PASS |				\
	...
	if (c->sa_clones > 0) {
		...
		if (free_clone == NULL)  {
			libreswan_log("AA_2019 %s %d no free clone connection for clonse = %u", __func__, __LINE__, c->sa_clones);
			return false;
		} else {
			dbg("AA_2019 %s %d switch connection from %s to %s",
